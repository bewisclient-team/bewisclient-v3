...
class Button(val text: String, val onClick: (Button) -> Unit, tooltip: Translation? = null, val selected: (() -> Boolean)? = null, var dark: Boolean = false) : TooltipHoverable(tooltip) {
    ...
    override fun render(screenDrawing: ScreenDrawing, mouseX: Int, mouseY: Int) {
        ...
        screenDrawing.fillWithBorderRounded(getX(), getY(), getWidth(), getHeight(), 5, 0xFFFFFF, (hoverAnimation["hovering"] * 1 + 1) * if (dark) 0.05f else 0.15f, 0xAAAAAA, if (selected?.invoke() == true) 0.5f else 0f)
        screenDrawing.push()
