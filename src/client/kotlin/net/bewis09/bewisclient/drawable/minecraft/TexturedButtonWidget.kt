...
open class TexturedButtonWidget(x: Int, y: Int, width: Int, height: Int, val normalTexture: Identifier, val selectedTexture: Identifier, pressAction: OnPress, val text: Component) : Button(x, y, width, height, CommonComponents.EMPTY, pressAction, DEFAULT_NARRATION), ClientInterface {
    ...
    override fun /*[@]*/extractContents/*[!@]*/(context: GuiGraphics, mouseX: Int, mouseY: Int, deltaTicks: Float) {
        val screenDrawing = ScreenDrawing(context, client.font)
        screenDrawing.drawTexture(if (this.isHovered) selectedTexture else normalTexture, this.x, this.y, 0f, 0f, this.width, this.height, this.width, this.height, (0xFFFFFF alpha this.alpha))
        screenDrawing.drawText(text, this.x + this.width / 2, this.y + 5) {
            color = Color.WHITE
