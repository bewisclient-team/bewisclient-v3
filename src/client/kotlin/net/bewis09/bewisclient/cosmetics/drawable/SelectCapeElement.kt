...
class SelectCapeElement(val identifier: CosmeticIdentifier, val cosmetic: Cosmetic) : TooltipHoverable() {
    ...
    override fun render(screenDrawing: ScreenDrawing, mouseX: Int, mouseY: Int) {
        ...
        screenDrawing.fillRounded(x, y, width, height, 5, OptionsMenuSettings.themeColor.get().getColor() alpha (selected["selected"] * 0.3f + hoverAnimation["hovering"] * 0.15f + 0.1f))

        screenDrawing.drawCape(cosmetic.getIdentifier(), x + 8, y + 8, width - 16, height - 25)
        screenDrawing.drawCenteredText(`snake_toWord With Spaces`(identifier.id), x + width / 2, y + height - 13, 0.5f within (Color.WHITE to OptionsMenuSettings.themeColor.get().getColor()))

        if (CosmeticLoader.elytraCosmetics.contains(identifier)) {
            screenDrawing.fillWithBorderRounded(x + width - 20, y, 20, 20, 5, 0.3f within (Color.BLACK to OptionsMenuSettings.themeColor.get().getColor()), 0.2f within ((selected["selected"] within (Color.DARK_GRAY to Color.WHITE)) to OptionsMenuSettings.themeColor.get().getColor()), topLeft = false, bottomRight = false)
            screenDrawing.drawTexture(createIdentifier("textures/item/elytra.png"), x + width - 18, y + 2, 16, 16)
        }

        screenDrawing.drawBorderRounded(x, y, width, height, 5, 0.2f within ((selected["selected"] within (Color.DARK_GRAY to Color.WHITE)) to OptionsMenuSettings.themeColor.get().getColor()))
    }
