...
class ShulkerBoxTooltipComponent(val data: Data) : TooltipComponent {
    ...
    override fun drawItems(textRenderer: TextRenderer, x: Int, y: Int, width: Int, height: Int, context: DrawContext) {
        ...
        screenDrawing.fill(x + 1, y + 1, getWidth(textRenderer) - 2, getHeight(textRenderer) - 7, data.color multiplyColor 0xC3C3C3, 1f)
        screenDrawing.fill(x + 4, y + 4, getWidth(textRenderer) - 8, getHeight(textRenderer) - 13, data.color, 1f)
        var i = 0
        for (k in 0..2) {
            for (l in 0..8) {
                val n = x + l * 18 + 9
    ...
    private fun drawSlot(x: Int, y: Int, index: Int, screenDrawing: ScreenDrawing) {
        ...
        screenDrawing.drawTexture(Identifier.of("bewisclient", "textures/gui/shulker_box/slot.png"), x, y, 0f, 0f, 18, 18, 18, 18, data.color, 1f)
        screenDrawing.drawItemStackWithOverlay(itemStack, x + 1, y + 1)
    }

    data class Data(val color: Int, val container: DefaultedList<ItemStack>) : TooltipData
