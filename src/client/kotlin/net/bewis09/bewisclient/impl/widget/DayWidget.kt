...
object DayWidget : LineWidget() {
    ...
    override fun getCustomWidgetDataPoints(): List<CustomWidget.WidgetStringData> = listOf(
        CustomWidget.WidgetStringData("day", "In-Game Day", "The current in-game day", { getText() }),
        CustomWidget.WidgetStringData("real_day", "Real-Life Day", "The current real-life day", { DateFormat.getDateInstance(DateFormat.SHORT, it?.let { Locale.forLanguageTag(it) } ?: Locale.getDefault()).format(Date.from(Instant.now())) }, "Use a language tag (e.g., \"en-US\") to format the date accordingly")
    )
}