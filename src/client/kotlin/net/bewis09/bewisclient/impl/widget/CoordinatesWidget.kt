...
object CoordinatesWidget : LineWidget() {
    ...
    val coordinatesWidgetTranslation =
        Translation("widget.coordinates_widget.name", "Coordinates Widget")
    val coordinatesWidgetDescription =
        Translation(
            "widget.coordinates_widget.description",
            ...
            if (showBiome.get())
                let {
                    ...
                    return@let (if (colorCodeBiome.get()) biomeCodes[biome]
                            ...
                            Text.translatable(biome.toTranslationKey("biome"))
                                .string
                }
            else null,
        )
    ...
    fun getYawPart(): Int =
        (((MinecraftClient.getInstance().player!!.yaw / 45 - 112.5).toInt()) % 8).let {
    ...
    override fun appendSettingsRenderables(list: ArrayList<Renderable>) {
        ...
        list.add(
            colorCodeBiome.createRenderable(
                "widget.coordinates_widget.color_code_biome",
                "Color Code Biome"
            )
        )
        list.add(
            showDirection.createRenderable(
                "widget.coordinates_widget.show_direction",
                "Show Direction"
            )
        )
        list.add(
            showCoordinateChange.createRenderable(
                "widget.coordinates_widget.show_coordinate_change",
                "Show Coordinate Change",
                "Shows how your coordinates will change if you move forward"
            )
        )
    ...
    override fun render(screenDrawing: ScreenDrawing) {
        ...
        if (showDirection.get()) {
            val direction =
                if (MinecraftClient.getInstance().player == null) "SW"
                else
                    when (getYawPart()) {
                        0 -> "S"
                        1 -> "SW"
            ...
            val text = "- $direction -"
            if (shadow.get())
                screenDrawing.drawRightAlignedTextWithShadow(
                    text,
                    getWidth() - paddingSize.get(),
                    paddingSize.get(),
                    textColor.get().getColor(),
                    1f
                )
            else
                screenDrawing.drawRightAlignedText(
                    text,
                    getWidth() - paddingSize.get(),
                    paddingSize.get(),
                    textColor.get().getColor(),
